...
def analyze_picture(filepath, model_emotion, model_gender, path, window_size, window_name='static'):
    cv2.namedWindow(window_name, WINDOW_NORMAL)
    cv2.namedWindow(window_name, WINDOW_NORMAL)
    if window_size:
        width, height = window_size
    ...
    #write the emotion(s) to a text file
    path = "../data/sample/"
    emotionsFile = open(path + "emotions.txt","w+") 
    ...
    for normalized_face, (x, y, w, h) in find_faces(image):
        ...
        emotionsFile.write(emotions[emotion_prediction[0]] + "\n")
        print("wrote to emotions file")
    cv2.imshow(window_name, image)

    #save the output image in the data/sample/output folder with the same name
        ...
        while run_loop:
            ...
            if file_name == "q" or file_name == "quit":
                ...
                if os.path.isfile(path):
                    TEST += 1
                    print(TEST)
                    analyze_picture(filepath, fisher_face_emotion, fisher_face_gender, path, window_size=(1280, 720), window_name=window_name)
                else:
